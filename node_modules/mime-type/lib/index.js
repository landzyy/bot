...
MimeType.prototype.lookup = function (aPath) {
  ...
  if (aPath && (0, _utilEx.isString)(aPath)) {
    ...
    if (extension) {
      if (/[*?!+|{]/.test(extension)) {
        result = Object.keys(this.types).filter(name => (0, _micromatch.isMatch)(name, extension));
...
MimeType.prototype.glob = function (pattern) {
  ...
  result = Object.keys(this).filter(function (name) {
    return (0, _micromatch.isMatch)(name, pattern);
  });
  return result;
};
  ...
  for (k in ref) {
    ...
    if (this.hasOwnProperty(k)) {
      if ((0, _utilEx.isFunction)(filter)) {
        ...
        if ((0, _micromatch.isMatch)(k, filter)) {
          this["delete"](k);
          result++;
        }
      } else {
